...
export default function AccountsList({ userId, type, isMainAccount }: AccountsListProps) {
  ...
  const handleFollow = async (accountId: string, username: string, profilePicUrl: string | null) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        Alert.alert(t('accountsList.errorTitle'), result.error || t('accountsList.failedToFollow'));
      }
    } catch (error) {
      Alert.alert(t('accountsList.errorTitle'), t('accountsList.failedToFollow'));
    } finally {
  ...
  const handleUnfollow = async (accountId: string, username: string) => {
    Alert.alert(t('accountsList.unfollowTitle'), t('accountsList.unfollowMessage', { username }), [
      { text: t('accountsList.cancel'), style: 'cancel' },
      {
        text: t('accountsList.unfollow'),
        style: 'destructive',
        onPress: async () => {
          setLoadingAccountId(accountId);
          try {
            ...
            if (result.success) {
              ...
              Alert.alert(t('accountsList.errorTitle'), result.error || t('accountsList.failedToUnfollow'));
            }
          } catch (error) {
            Alert.alert(t('accountsList.errorTitle'), t('accountsList.failedToUnfollow'));
          } finally {
            setLoadingAccountId(null);
          }
        },
  ...
  const title = type ? t(getAccountListLabel(type, isMainAccount)) : '';
      ...
      <View className="flex-1 p-4">
        ...
        {isLoading ? (
          <View className="flex-1 items-center justify-center">
            <Text className="text-gray-500">{t('accountsList.loading')}</Text>
          </View>
        ) : filteredAccounts.length === 0 ? (
          <View className="flex-1 items-center justify-center">
            <Text className="text-gray-500">{t('accountsList.noAccountsFound')}</Text>
          </View>
        ) : (
          <FlashList
            ...
            renderItem={({ item: account }) => {
              ...
              if (actionType === 'follow' && !isCompleted) {
                actionProps = {
                  actionLabel: t('accountsList.follow'),
                ...
                actionProps = {
                  actionLabel: t('accountsList.unfollow'),
                ...
                actionProps = {
                  actionLabel: actionType === 'follow' ? t('accountsList.following') : t('accountsList.unfollowed'),
                  actionVariant: 'secondary' as const,
                };
              }
