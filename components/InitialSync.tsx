...
export default function InitialSync({
  ...
  const step2Label =
    followingDisabled && followersDisabled
      ? t('components.initialSync.step2SyncingInstagram')
      : followersDisabled
        ? t('components.initialSync.step2SyncingFollowing')
        : followingDisabled
          ? t('components.initialSync.step2SyncingFollowers')
          : t('components.initialSync.step2SyncingInstagram');
  ...
  return (
    <View className="flex-1 items-center justify-center p-4">
      ...
      <Text className="mt-4 px-12 font-roboto-extrablack text-4xl tracking-tighter">
        {t('components.initialSync.title')}
      </Text>
      <Text className="font-roboto-regular mt-4 px-12 text-center text-lg tracking-tighter">
        {followingDisabled && followersDisabled
          ? t('components.initialSync.subtitleFetchingProfile')
          : t('components.initialSync.subtitleMayTakeMinute')}
      </Text>

      <View className="mt-6 w-full gap-4">
        <SyncStep label={t('components.initialSync.stepFetchingProfile')} state={step1State} />
        {(!followingDisabled || !followersDisabled) && (
          <SyncStep label={step2Label} state={step2State} />
        )}
        <SyncStep label={t('components.initialSync.stepAnalyzing')} state={step3State} />
      </View>
