...
const INSIGHT_ROWS: InsightRow[] = [
  { labelKey: 'insights.newFollowers.label', blurredValue: '24', locked: true },
  { labelKey: 'insights.lastActivity.label', blurredValueKey: 'insights.lastActivity.value', locked: true },
  { labelKey: 'insights.flagged.label', blurredValue: '18', locked: true },
  { labelKey: 'insights.mostActive.label', locked: false },
];
    ...
    <View className="flex-1">
            ...
            <View className="ml-8 flex-1 flex-row gap-6">
              {mediaCount !== null && mediaCount !== undefined && (
                <View className="items-center">
                  ...
                  <Text className="font-roboto text-sm text-gray-500">{t('stats.posts')}</Text>
                </View>
              )}
              {followingCount !== undefined && (
                <View className="items-center">
                  <Text className="font-roboto-bold text-2xl">{formatCount(followingCount)}</Text>
                  <Text className="font-roboto text-sm text-gray-500">{t('stats.following')}</Text>
                </View>
              )}
              {followersCount !== undefined && (
                <View className="items-center">
                  <Text className="font-roboto-bold text-2xl">{formatCount(followersCount)}</Text>
                  <Text className="font-roboto text-sm text-gray-500">{t('stats.followers')}</Text>
                </View>
              )}
            </View>
          </View>
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }} pointerEvents="box-none">
        <LinearGradient
          ...
          <Button label={t('reveal.showEverything')} onPress={handleContinue} />
