...
const FAKE_ACCOUNTS = [
  {
    ...
    time: 'onboarding.trackSearch.timeYesterday',
  },
  {
    id: '2',
    username: 'sarah_designs',
    image: require('~/assets/profile_0.jpg'),
    time: 'onboarding.trackSearch.timeHoursAgo_2',
  },
  {
    id: '3',
    username: 'mike.travels',
    image: require('~/assets/profile_1.jpg'),
    time: 'onboarding.trackSearch.timeHoursAgo_5',
  },
];
...
export default function TrackSearch({ onNext, savedResult, onResultFetched }: TrackSearchProps) {
  ...
  if (savedResult) {
    return (
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 24 }}>
        <Text className="mb-8 text-center font-roboto-black text-4xl">{t('onboarding.trackSearch.resultTitle')}</Text>
        ...
        <View className="mt-8">
          <Button label={t('onboarding.trackSearch.cta')} onPress={() => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setError(t('onboarding.trackSearch.errorNotFound'));
    } finally {
    ...
    <KeyboardAvoidingView
      ...
      <View className="flex-1 items-center justify-center px-4">
        ...
        <View className="mt-6 w-full">
          <Button
            label={t('onboarding.trackSearch.cta')}
            loading={isSearching}
