...
export default function UsernameSearch({
  ...
  if (savedResult) {
    return (
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 24 }}>
        <Text className="mb-8 text-center font-roboto-black text-4xl">{t('onboarding.usernameSearch.resultTitle')}</Text>
            ...
            <View key={account.id} className="overflow-hidden rounded-2xl">
              <View className="flex-row items-center gap-3 bg-white p-4">
                <View className="relative h-12 w-12 overflow-hidden rounded-full">
                  {Platform.OS === 'android' ? (
                    <Animated.Image source={account.image} className="h-12 w-12 rounded-full" style={{ filter: 'blur(6px)' }} />
                  ) : (
                    <>
        ...
        <View className="mt-8">
          <Button label={t('onboarding.usernameSearch.cta')} onPress={() => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setError(t('onboarding.usernameSearch.errorNotFound'));
    } finally {
    ...
    <KeyboardAvoidingView
      ...
      <View className="flex-1 items-center justify-center px-4">
        ...
        <View className="mt-6 w-full">
          <Button
            label={t('onboarding.usernameSearch.cta')}
            loading={isSearching}
