...
export default function Index() {
  ...
  const formatLastSyncTime = (timestamp: string | null): string => {
    ...
    if (diffMins < 1) return t('sync.updatedJustNow');
    if (diffMins < 60) return t('sync.updatedMinsAgo', { count: diffMins });
    if (diffHours < 24) return t('sync.updatedHrsAgo', { count: diffHours });
    return t('sync.updatedDaysAgo', { count: diffDays });
  };
  ...
  const handleRefresh = () => {
    ...
    if (isCoolingDown) {
      Alert.alert(t('refresh.pausedTitle'), t('refresh.pausedMessage'));
      return;
    }

    Alert.alert(t('refresh.confirmTitle'), t('refresh.confirmMessage'), [
      { text: t('refresh.cancel'), style: 'cancel' },
      { text: t('refresh.confirm'), onPress: sync },
    ]);
  };
