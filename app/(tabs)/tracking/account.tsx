...
export default function TrackingAccount() {
  ...
  const handleStopTracking = () => {
    ...
    Alert.alert(
      t('trackingAccount.stopTrackingTitle'),
      t('trackingAccount.stopTrackingConfirm', { username: instagram?.username || username }),
      [
      { text: t('trackingAccount.cancel'), style: 'cancel' },
      {
        text: t('trackingAccount.stopTracking'),
        style: 'destructive',
        onPress: async () => {
          try {
            ...
            Alert.alert(t('trackingAccount.errorTitle'), t('trackingAccount.stopTrackingError'));
          }
        },
      },
    ]);
                ...
                <View className="border-b border-gray-100 p-4">
                  <View className="flex-row items-center justify-between">
                    <Text className="font-roboto-medium text-base text-gray-900">
                      {t('trackingAccount.syncFollowingActivity')}
                    </Text>
                    <Switch
                  ...
                  {followingWarn && (
                    <Text className="mt-2 font-roboto-regular text-sm text-gray-400">
                      {t('trackingAccount.rateLimitWarning')}
                    </Text>
                  )}
                </View>
                <View className="border-b border-gray-100 p-4">
                  <View className="flex-row items-center justify-between">
                    <Text className="font-roboto-medium text-base text-gray-900">
                      {t('trackingAccount.syncFollowersActivity')}
                    </Text>
                    <Switch
