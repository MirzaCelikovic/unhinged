...
export default function TrackModal() {
  ...
  const handleContinue = async (inputUsername: string) => {
    ...
    if (!trimmedUsername) {
      setError(t('trackingTrack.invalidUsername'));
      return;
    }

    if (!account?.uuid) {
      setError(t('trackingTrack.accountNotReady'));
      return;
    }
    ...
    try {
      ...
      if (result.isPrivate && !result.followedByViewer) {
        ...
        setError(t('trackingTrack.notFollowing'));
        return;
      }
      ...
      if (result.isVerified) {
        ...
        setError(t('trackingTrack.verifiedNotSupported'));
        return;
      }
      ...
      if (err instanceof Error && err.message.includes('not found')) {
        setError(t('trackingTrack.accountNotFound'));
      } else {
        setError(t('trackingTrack.lookupFailed'));
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleStartTracking = async () => {
    ...
    try {
      ...
      if (isCoolingDown) {
        Alert.alert(
          t('trackingTrack.pausedTitle'),
          t('trackingTrack.pausedMessage')
        );
        router.back();
        return;
      }
      ...
      console.log('❌ Error:', err);
      setError(t('trackingTrack.addFailed'));
    } finally {
      setIsLoading(false);
    }
  };
