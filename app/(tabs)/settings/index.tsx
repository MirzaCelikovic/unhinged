  ...
  return (
    <Modal visible={visible} animationType="fade" statusBarTranslucent>
      <View className="flex-1 items-center justify-center bg-background">
        ...
        <Text className="mt-4 font-roboto-medium text-lg text-black">{t('settings.deleting')}</Text>
      </View>
    </Modal>
  );
});
...
export default function Settings() {
  ...
  const handleDisconnect = () => {
    Alert.alert(
      t('settings.alert.disconnect.title'),
      t('settings.alert.disconnect.message'),
      [
        { text: t('common.cancel'), style: 'cancel' },
        { text: t('settings.alert.disconnect.confirm'), style: 'destructive', onPress: disconnect },
      ]
    );
  };
  ...
  const handleDeleteAccount = () => {
    Alert.alert(
      t('settings.alert.delete.title'),
      t('settings.alert.delete.message'),
      [
        { text: t('common.cancel'), style: 'cancel' },
        {
          text: t('settings.alert.delete.confirm'),
          style: 'destructive',
          onPress: async () => {
            setIsDeleting(true);
    ...
    <View className="flex-1 bg-background">
      ...
      <ScrollView className="flex-1">
        <View className="px-4 pb-4 pt-2">
          {/* Settings Section */}
          <View className="mt-4">
            <Text className="mb-3 font-roboto-bold text-lg text-black">{t('settings.sectionTitle')}</Text>
